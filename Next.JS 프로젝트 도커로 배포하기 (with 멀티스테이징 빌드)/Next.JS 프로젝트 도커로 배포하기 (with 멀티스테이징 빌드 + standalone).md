...
## 3. 결과
![[Pasted image 20250512021323.png]]

처음에 1.34GB였던 도커 이미지가 멀티 스테이징 빌드와 standalone 모드를 적용한 후 182MB로 줄어들었다. 이건 무려 약 86.4%나 감소한 것이다!

이렇게 이미지 용량이 대폭 줄어들면서 다음과 같은 효과를 얻을 수 있었다:

1. **서버 공간 효율적 사용**: 내가 사용하는 임시 서버의 간당간당했던 용량 문제가 크게 해소됨
2. **배포 시간 단축**: 훨씬 작은 이미지로 인해 업로드 및 다운로드 시간이 크게 줄어듦
3. **컨테이너 시작 시간 개선**: 더 작은 이미지는 더 빠르게 로드되고 시작됨
4. **자원 사용 최적화**: 컨테이너가 차지하는 메모리와 디스크 공간이 줄어들어 서버 리소스를 효율적으로 사용

이 정도 용량 감소면 같은 서버에서 훨씬 더 많은 컨테이너를 실행할 수 있고, 무엇보다 내가 사용하던 간당간당한 서버에서도 여유롭게 운영할 수 있게 되었다.

스프링 부트 이미지(250MB)보다도 더 작은 크기로 줄어든 것을 보면 최적화가 정말 효과적이었다고 할 수 있다. 결론적으로 Next.js 애플리케이션을 Docker로 배포할 때는 멀티 스테이징 빌드와 standalone 모드를 함께 사용하는 것이 거의 필수라고 생각된다.